...
declare -r script_USAGE="usage: ${script_PROGNAME} [options] <INFILE >OUTFILE"
declare -r script_DESCRIPTION='Script preprocessor for MBFL.'
...
# The file system pathname  in which the file "libmbfl.bash" is installed.  This  is the default value
# of the option "--libmbfl".
#
declare -r DEFAULT_LIBMBFL=${pkgdatadir}/libmbfl.bash

# The file system pathname in which the file "libmbfl-tests.bash" is installed.
#
declare -r DEFAULT_LIBMBFLTEST=${pkgdatadir}/libmbfl-tests.bash

# A string representing  command line options for GNU  m4: the symbols definitions.  It  is built by
# appending the definitions from the command line option "--define" as:
#
#   --define=<symbol>=<value>
#
declare M4_SYMBOL_DEFINITIONS

# A string  representing command  line options  for GNU  m4: the  macro libraries.   It is  built by
# appending the definitions from the command line option "--library".
#
declare M4_LIBRARY_OPTIONS

# A string representing command  line options for GNU m4: the search path  for include files.  It is
# built by appending the definitions from the command line option "--include" as:
#
#   --include=<directory>
#
declare M4_INCLUDE_OPTIONS
...
mbfl_declare_option PRINT_LIBPATH		no '' print-libdir        noarg \
		    'print the librarires installation directory'
mbfl_declare_option PRINT_LIBMBFL		no '' print-libmbfl       noarg \
		    'print the full installation pathname of the library "libmbfl.bash"'
mbfl_declare_option PRINT_LIBMBFL_TESTS		no '' print-libmbfl-tests   noarg \
		    'print the full installation pathname of the library "libmbfl-tests.bash"'
mbfl_declare_option PRINT_LIBMBFL_UTILS		no '' print-libmbfl-utils noarg \
		    'print the full installation pathname of the library "libmbfl-utils.bash"'
mbfl_declare_option PRINT_LIBMBFL_ARCH		no '' print-libmbfl-arch  noarg \
		    'print the full installation pathname of the library "libmbfl-arch.bash"'
mbfl_declare_option PRINT_LIBMBFL_AT		no '' print-libmbfl-at    noarg \
		    'print the full installation pathname of the library "libmbfl-at.bash"'
mbfl_declare_option PRINT_LIBMBFL_PASSWORDS	no '' print-libmbfl-passwords    noarg \
		    'print the full installation pathname of the library "libmbfl-passwords.bash"'
mbfl_declare_option PRINT_LIBMBFL_GIT		no '' print-libmbfl-git    noarg \
		    'print the full installation pathname of the library "libmbfl-git.bash"'
...
mbfl_declare_exit_code 3 invalid_semantic_version
...
function script_option_update_print_libpath () {
    printf '%s\n' "$MBFL_INSTALLDIR"
    exit_success
}
function script_option_update_print_libmbfl () {
    printf '%s/libmbfl.bash\n' "$MBFL_INSTALLDIR"
    exit_success
}
function script_option_update_print_libmbfl_tests () {
    printf '%s/libmbfl-tests.bash\n' "$MBFL_INSTALLDIR"
    exit_success
}
function script_option_update_print_libmbfl_utils () {
    printf '%s/libmbfl-utils.bash\n' "$MBFL_INSTALLDIR"
    exit_success
}
function script_option_update_print_libmbfl_arch () {
    printf '%s/libmbfl-arch.bash\n' "$MBFL_INSTALLDIR"
    exit_success
}
function script_option_update_print_libmbfl_at () {
    printf '%s/libmbfl-at.bash\n' "$MBFL_INSTALLDIR"
    exit_success
}
function script_option_update_print_libmbfl_passwords () {
    printf '%s/libmbfl-passwords.bash\n' "$MBFL_INSTALLDIR"
    exit_success
}
function script_option_update_print_libmbfl_git () {
    printf '%s/libmbfl-git.bash\n' "$MBFL_INSTALLDIR"
    exit_success
}

function script_option_update_check_version () {
    local -i mbflpp_RV

    if mbfl_semver_compare_var mbflpp_RV "$SEMANTIC_VERSION" "$script_option_CHECK_VERSION"
    then
	if ((mbflpp_RV >= 0))
	then
	    mbfl_message_verbose_printf \
		'sucessful check: the given semantic version "%s" is less than, or equal to, the MBFL package version "%s"\n' \
		"$script_option_CHECK_VERSION" "$SEMANTIC_VERSION"
	    exit_success
	else
	    mbfl_message_verbose_printf \
		'sucessful check: the given semantic version "%s" is greater than the MBFL package version "%s"\n' \
		"$script_option_CHECK_VERSION" "$SEMANTIC_VERSION"
	    exit_failure
	fi
    else
	mbfl_message_error_printf 'invalid semantic version: "%s"' "$script_option_CHECK_VERSION"
	exit_because_invalid_semantic_version
    fi
}

function script_after_parsing_options () {
    if mbfl_string_equal "$script_option_OUTPUT" '-'
    then script_option_OUTPUT=/dev/stdout
    fi
}
...
function main () {
    ...
    local PREPROCESSOR=${pkgdatadir}/libmbfl-preprocessor.m4
    local OUTPUT

    M4_FLAGS+=" ${M4_INCLUDE_OPTIONS} ${M4_SYMBOL_DEFINITIONS}"
    if mbfl_string_equal "$script_option_NO_PREPROCESSOR" 'no' && mbfl_file_is_readable "$PREPROCESSOR"
    then M4_FLAGS+=" ${PREPROCESSOR}"
    fi
    M4_FLAGS+=" ${M4_LIBRARY_OPTIONS}"

    if ! {
	    # If there are  no arguments on the command  line: read input from stdin.   If there are
	    # arguments on the command line: they must all by files.
            if ((0 == ARGC))
    	    then program_m4 -
            elif mbfl_argv_all_files
    	    then program_m4 "${ARGV[@]}"
    	...
    	} | filter_page_separators | {
	    # If we  send the output to  a "bash" process: there  is no need to  prepend the shebang
	    # line.
            if mbfl_string_equal "$script_option_EVAL" 'yes'
...
# By inverting the match rule  for "grep": we include in the output all the  lines that do not match
# the given regexp.  We  want the lines starting with "#" to be  dropped, unless the character after
# the "#" is "!".
#
...
function filter_page_separators () {
    program_grep --invert-match --regexp='^'$'\f''$'
}
...
function program_m4 () {
    mbfl_program_exec "/usr/bin/m4" \
		      ...
		      -D__MBFL_LIBMBFL_INSTALLATION_PATHNAME__="$script_option_LIBMBFL"	\
		      $M4_FLAGS "$@"
}
