...
# Embedding the library is the safest way.  Does not really matter if this script becomes big.
# libmbfl.m4 --
#
# Part of: Marco's BASH Functions Library
...
local mbfl_a_variable_DATE
mbfl_variable_alloc mbfl_a_variable_DATE
local  $mbfl_a_variable_DATE
local -n DATE=$mbfl_a_variable_DATE

mbfl_program_found_var $mbfl_a_variable_DATE date || exit_because_program_not_found
...
declare -r bindirx="${exec_prefix}/bin"
declare -r datarootdir="${prefix}/share"
declare -r datadir="${datarootdir}"
declare -r pkgdatadir="${datarootdir}/mbfl"

declare -r PACKAGE_VERSION='3.0.0-devel.7'
declare -r PACKAGE_DATADIR=$pkgdatadir
...
# "loader.sh".
#
declare -r DEFAULT_MBFL_LIBRARY=${pkgdatadir}/libmbfl.sh

# The absolute pathname of the installed MBFL utilities library.
#
declare -r DEFAULT_MBFL_UTILS_LIBRARY=${pkgdatadir}/libmbflutils.sh
...
mbfl_declare_option MBFL_UTILS_LIBRARY "$DEFAULT_MBFL_UTILS_LIBRARY" '' mbfl-utils-library witharg "pathname of the MBFL utilities library"
...
function main () {
    ...
    if ! {
    	    ...
    	    then program_m4 \
		     ...
		     -D__MBFL_UTILS_LIBRARY__="$script_option_MBFL_UTILS_LIBRARY" \
		     ${M4_FLAGS} -
            else program_m4 \
		     -D__PACKAGE_VERSION__="$PACKAGE_VERSION" -D__PACKAGE_DATADIR__="$PACKAGE_DATADIR" \
		     -D__MBFL_LIBRARY__="$script_option_MBFL_LIBRARY" \
